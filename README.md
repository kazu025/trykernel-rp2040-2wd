...
- タスク起床待ち・起床
- セマフォ
- FIFOミューテックス（所有者管理・タイムアウト、優先度継承なし）
- イベントフラグ
- タスク付帯同期
- 割り込み・例外コンテキストの判定
...
LCD温度表示、MPU割り込み処理、INA226電力監視、UARTコマンドは同じI2C0を共有します。各I2CトランザクションをFIFOミューテックスで保護し、ロック所有者だけが解除できます。
...
共有資源の排他制御向けに、FIFO方式の基本ミューテックスを実装しています。
...
- `tk_cre_mtx()`：作成。最大8個。属性は`TA_TFIFO`のみ対応。
- `tk_loc_mtx()`：ロック。空いていれば取得し、使用中ならFIFO順で待機。
- `tk_unl_mtx()`：所有者が解除。先頭の待ちタスクに直接所有権を移すため、別タスクが先に取得することはありません。
...
優先度継承・優先度上限・再帰ロック・削除APIは未対応です。低優先度の所有者が中優先度タスクに実行を妨げられる優先度逆転は、この基本版では解決しません。

### 検証

`make test-mutex`で、実際のミューテックス・待ち行列・タイマ・タスク終了コードをホスト上で検証します。割り込み制御とコンテキスト切り替えは模擬しているため、実機のPendSVやI2C信号の検証にはなりません。

テスト対象は引数・コンテキスト検査、所有者管理、二重ロック、ポーリング、有限タイムアウト後の再取得、2タスクへのFIFO引き渡し、横取り防止、所有タスク終了時の全ロック解放、生成上限です。

実機確認は以下の手順で行います（この変更では未実施）。
...
- I2CトランザクションはFIFOミューテックスでタスク間排他制御しています。
